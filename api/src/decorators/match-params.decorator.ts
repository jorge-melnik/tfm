...
export default fp(async (fastify: FastifyInstance) => {
  fastify.decorate('matchParamsWithBody', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!params || !body) {
      ...
      throw new BadRequestError('Petición inválida: faltan datos de validación');
    }
    ...
    for (const field of Object.keys(params)) {
      ...
      if (paramValue !== bodyValue) {
        ...
        throw new BadRequestError(`El valor de '${field}' en la URL no coincide con el del cuerpo`);
      }
    }
  });
});
